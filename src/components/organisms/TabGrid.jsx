...
const TabGrid = ({ tabs, isLoading, onStartSession }) => {
  if (isLoading) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-xl font-display font-semibold text-white">Active Tabs</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
        {tabs.map((tab, index) => (
          <TabPreview key={tab.id} tab={tab} index={index} />
        ))}
      </div>
    </motion.div>
  );
